...
scen_data <- run_forecast(historic_values = hist_vals,
                          ...
                          list_of_years = years,
                          list_of_regions = regions_filter,
                          scenario_name = "building_pri_high")
...
export_df <- rbind(export_df, scen_data)
#Add same scenario also for low scenario
scen_data <- scen_data %>% mutate(scenario = "building_pri_low")
export_df <- rbind(export_df, scen_data)
...
#AGGREGATE UTILITARIAN
#GET GROWTH RATES
end_val <- hist_vals %>% filter(year == 2020) %>% mutate(value = value + 3000) %>%
